...
<body onload=display_ct();>
	<header>
		<nav class="navbar navbar-light" style="background-color: #faffafef;">
			<div class="container">
				...
				<a class="navbar-brand fw-bolder" href="student_login.php">
	...
	<main>
		<p class="fw-bolder mt-4" style="text-align: center; font-size:40px;">Student Login</p>
			...
			<div class="login">
				<form action="student_login.php" method="post">
					...
					<input type="submit" value="Login" name="login" class="fs-6 fw-bold border-1 rounded-2 px-1"
						style="background-color: #5584AC; color: rgba(245, 245, 245, 0.925);"><br><br>
				</form>
    ...
    session_start();
        ...
        $select = mysqli_query($con, "SELECT * FROM student_user WHERE id = '$Username'  AND studentpass = '$password'");
        $row = mysqli_fetch_array($select);

        if(is_array($row))
        {
            $_SESSION['username'] = $row['id'];
            $_SESSION['password'] = $row['studentpass'];
        }
        else{
...
<!-- INSERT INTO `admin_user` (`sl`, `username`, `password`) VALUES (NULL, 'munem', 'munem'); -->